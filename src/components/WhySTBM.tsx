...
export function WhySTBM() {
  return (
    <section id="why-stbm" className="py-20 bg-gradient-to-br from-neutral-10 to-neutral-9 text-white relative overflow-hidden">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
          {reasons.map((reason, index) => (
            <div key={index} className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-lg p-6">
              ...
              <p className="text-neutral-1 text-sm">{reason.description}</p>
            </div>
          ))}
        </div>
      </div>
